...
There are four conclusions from @f1agg4. 

First, @tbl-f1aggov4 mostly has higher values than @tbl-f1aggms4, suggesting that as the detail of the target clusters increases, so does the importance of the building footprints quality.

Second, overall the scores in @tbl-f1agg4 are lower than the scores in @tbl-f1agg across all models. This shows that the prediction task becomes more difficults. It also suggests that we do reasonable misclassifications - one class in Level 4  is often missclasified for another Level 4 class, but both have the same Level 3 class.

Third, the same pattern as regards the models holds as in @f1agg - spatial leakege is present everywhere.

Fourth, heterogeinety of urban form is same as @f1agg.
...
@tbl-f1classov4 has consistenly higher values than @tbl-f1classms4, in contrast to the pattern in @tbl-f1class, where only the **Central Urban Developments** class showed a significant difference. The one exception is Slovakia.

Similarly to @tbl-f1class, the heterogeneity of urban form is pronounced in the detailed classes, as evidenced by large differences in F1 scores between countries.

Lastly, the spatial testing pattern holds across different scales of urban form dissimilarity and across classes. As the spatial stratification becomes more complex, the accuracy falls.
...
Since the street network is shared, the results show the importance of the street network and how it can make up for poor quality building footprints data.

Data from heterogenous sources can be used to identify urban morphological types.
Different types of characteristics and data quality are important for different types of urban form.



The results from the comparisons between the different testing data splitting strategies open new avenues of research.
The results also show some of the problems with reporting aggregate accuracy scores.
