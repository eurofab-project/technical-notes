...
We employed labels from the Spatial Signatures Framework[^2], a typology classifying British urban environments based on form (physical appearance) and function (usage). Spatial signatures effectively characterise urban areas, reflecting both visual form and operational functionality. Although our project specifically targets urban fabric classification based primarily on visible form—potentially simpler to predict—the form-specific classification scheme remains under development. Consequently, the comprehensive Spatial Signatures Framework currently serves as a proxy aligning closely with our project's urban characterisation objectives.
...
We used two versions of this dataset: one with 12 classes and another simplified version with 7 classes. The 12-class scheme consolidates the various urbanity categories into a single class, maintaining all other Spatial Signatures classes:
...
A significant issue in our dataset is class imbalance, with specific urban fabric classes being substantially underrepresented. This imbalance informed our decisions on model architecture and loss function selection, motivating exploration of specialised methods to handle imbalanced data.
...
Due to the significant class imbalance, particularly the underrepresentation of urban classes, we implemented a sliding-window augmentation strategy for classes comprising less than 10% of the dataset (all except *Countryside agriculture* and *Wild countryside*).

This sliding-window approach systematically shifted the sampling window horizontally and vertically by increments of 50, 100, 150, and 200 metres, significantly increasing the volume of available training data.

![Sliding Window Augmentation](../figures/results/img_aug.jpg){.nostretch fig-align="center" height="420"}

The images below illustrate class distributions before (left) and after (right) augmentation:
...
Including spatial context (H3 resolution) notably improved classification accuracy and F1 scores. This improvement shows the importance of spatial context in predicting urban fabric classes.
...
The overall overlap between the initial year (2016) and final year (2021) remained high at 88%. This confirms that urban fabric classes remained relatively stable across the study period. However, minor variations may indicate either genuine urban fabric evolution or model uncertainty, warranting further exploration.
...
The overall overlap between the start (2016) and end (2021) years remains high at 88%, further confirming the relative stability in the urban fabric classes across the entire study period. These changes may capture actual changes in the urban fabric, however, could also point to uncertainty in the classifier.
...
Class-specific analysis further highlighted particular urban fabric types prone to transitions. The figure below shows which classes experienced frequent transitions:
...
Example visualisations and datasets generated during verification exercises are also available in the EO repository.
...
pipeline.spatial_sig_prediction(
    geo_path= "../spatial_signatures/eo/data/example/london_25_25_grid_clipped.geojson",
    vrt_file= "../satellite_demoland/data/mosaic_cube/vrt_allbands/2017_combined.vrt",
    xgb_weights = "../spatial_signatures/classifier/k12_h5_slided_gb_xgb_model.bin",
    model_weights = "../satellite_demoland/models/satlas/weights/satlas-model-v1-lowres.pth",
    output_path= "../vjgo8416-demoland/spatial_signatures/eo/data/predictions/test_london_h6.parquet",
    h3_resolution=5
)`
```

More details and documentation can be found in the EO repository https://github.com/eurofab-project/eo/tree/main.
...
Here are some example visualisations from the dataset:
