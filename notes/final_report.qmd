...
The spatial layout of the physical elements of cities affects most activities their residents undertake. Accessability to different areas [@calafiore2023inequalities], overall economic performance[@WANG2024105043], cost of living [@QIU2022104358], as well as energy effeciency[@ZHU2022104576] and heat islands[@WU2025105314] are some of the areas where urban form is used to guide research, applications and policy. Due to the numerous patterns, scales and manifestations of urban form, researchers turn to classification as a method to reduce complexity and group the variations in urban form into classes. The resulting classification of urban form can provide valuable insights into the structure of cities and towns, guide targeted policy applications, and form the backbone of urban planning.
...
<!--Aims-->
Therefore, there is a need to: one, derive detailed classifications at scale from suboptimal data that does not have the same qualities as, for example, cadastre would have; and two, add a temporal dimension to the classification which shows how it evolves over time.

<!--Approach-->
This project develops a methodology to achieve these two objectives, and verifies it through a pilot study in Central Europe and the United Kingdom. The basis for our work are two models - one based on satellite-derived building footprints; and a second, based on a direct use of Sentinel-2 visible bands. The overall goal is to use the first machine learning model to produce training data at scale for the second AI model, since currently such datasets do not exist, and to finally use the AI model to derive a detailed temporal classification of urban form in Europe. Such a novel classification can improve urban-form related research and policy applications, by providing researchers, policy makers and practicioners with unprecedented detail and temporal information on urban form. To verify the validity of our approach and the usefulness of the results, we further carry out consultations with stakeholders from the private sector, local planning departments and government organisations.
...
The main aim of the morphological machine learning model is to show that the [@primus] urban fabric classification, built on cadastre data, can be reproduced using more widely available data sources - building footprints derived from satellite imary, volunatarily contributed building footrints, i.e. Openstreetmap building footprints, or combinations of both [@overture]. These models can be later used to expand the cadastral taxonomy and generate training data for the AI vision model, which will provide a temporal dimension of the classification. The study area covers five countries in Central Europe - Germany, Poland, Slovakia, Austria, Czechia. The bulk of the morphological work can be summarised as: 

1. Developing a scalable pipeline to generate morphological characteristics from alternative building footprints
2. Assignment of cadastral urban classes to the alternative building footprints ( the ground truth )
3. Training several models in different contexts and evaluating their ability to predict cadastral urban form classes , based on the features calculated in 1.
...
The street network is a direct download from Overture Maps Transportation theme, a processed subset of data from OpenStreetMap, which has global coverage and high quality data.
...
@fig-traintest shows the new train / test splits. The main goal with so many training configurations is to get detailed statistics about the expected model performance in realistic scenarios - predicting urban form in various types of new environemnts. Two models - a global random model and a global spatially explicit model - are trained to highlight the effects of spatial lag in model training and testing, while the contry specific models are trained to evaluate the ability of the models to generalise.

Furthermore, all experiments are repeated independely for each dataset - once for overture and once for microsoft building footprints. This is done to verify in which cases what data can be used to derive detailed urban form classifications.

The models for each experiment are HistogramBoostedTrees and share the same parameters - .

## Results

### Level 3 Performance scores
...
<!-- expand -->
The stakeholder work provides support for this.
