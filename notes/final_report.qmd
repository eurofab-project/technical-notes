...
author: "Krasen Samardzhiev, Barbara Metzler, Martin Fleischmann, \\ Daniel Arribas-Bel, Claudia Baranzelli, Alexandre Banquet"
...
The spatial layout of the physical elements of cities affects most activities their residents undertake. Accessability [@calafiore2023inequalities], overall economic performance[@WANG2024105043], cost of living [@QIU2022104358], as well as energy effeciency[@ZHU2022104576] and heat island effects [@WU2025105314] are some of the application areas where urban form is used to guide research, applications and policy. Due to the numerous patterns, scales and manifestations of urban form, researchers turn to classification as a method to reduce complexity and group the variations in urban form into classes. The derived classes vary depending on the application. Example coarse groups are continuous, discontinious and non-urban land [@corine]. More detailed groupings capture the characteristics of streets and buildings, for example organic and gridiron streets [@arinaldi2025]. These resulting classifications of urban form provide valuable insights into the structure of cities and towns, guide targeted policy applications, and form the backbone of urban planning.

However, detailed, scalable, and consistent classifications that reflecting the nature of the local data all at the same time are scarcely available. We often see conceptual classification, which has a tendency to oversimplify the structure when “zooming in”, loosing the interest of planners and policy-makers, i.e. [@corine, @lcz]. The classifications developed by key members of the EuroFab project team overcome these limitations [@primus ; @fleischmann2022geographical ; @fleischmann2022methodological], but are  dependent on the availability of high quality data capturing urban form, e.g. individual buildings and related street networks. The issue is that these are not always readily available, even within the context of generally data-rich countries of the European region. Furthermore, even if countries have high quality building data, they are typically missing information on the temporal evolution of the elements.
...
The main aim of the morphological machine learning model is to show that the [@primus] urban fabric classification, built on cadastre data, can be reproduced using more widely available data sources - building footprints derived from satellite imary, volunatarily contributed building footrints, i.e. Openstreetmap building footprints, or combinations of both [@overture]. This model can be later used to expand the cadastral taxonomy and generate training data for the AI vision model. 
...
![Comparison between buiding footprint data sources.](../figures/results/building_comparison.png){.nostretch fig-align="center" #fig-data}

The core datasets used in the analysis are two types of buidling footprints - Microsoft Satelite Building footprints  and OvertureMap building footprints - and street geometries from OvertuMaps.
The street network is a direct download from Overture Maps Transportation theme, a processed subset of data from OpenStreetMap, which has global coverage and high quality data. And example for Prague is shown in @fig-street.

@fig-data shows a comparison between the cadastral data, Microsoft building footprints and Overtumap building footprints.
...
OvertureMaps data is more detailed and a combination of datasets, it even includes some Microsoft footprints.
...
3. Calculating 52 morphological characteristics which capture aspects of all individual elements (i.e. building area or perimenter), as well as their interactions - for example, buildings per street or number of reachable ETCs within five topological steps of the street network.

This pipeline results in around 62 million ETCs with 52 morpholgical characteristics for the Microsoft data, and around 88 million ETCs with 52 morpholgical characteristics for the OvertureMaps data.
...
The taxonomy of ground truth classes from [@primus] and available as an interactive map online at [HiMOC](https://uscuni.org/himoc/) is presented in @tbl-taxonomy. All subsequent analysis focuses on Levels 3 and 4. The target labels from this are assigned to each building from the morphological pipeline based on spatial intersections.
