...
The core functionality for all of this was made available within open-source packages - momepy, libpysal, sgeop.
...
These results highlight the effect of the satellite derived building footprints that have been discussed in the Technical Note D3 . There are significanlty less subregions in the study area, since there are around 30 million less buildings, which are either missing or wrongly merged into a single polygon. In turn these issues affect the interpretation of the values of the calculated morphological characters. Some characters from the original classification even completely lose their meaning. For example, the number of adjacent buildings in the EuroFab dataset is much smaller and therefore all characters based on adjacency have a much more limited utility. This also affects, to a relatively smaller, but nevertheless important extent, characters that rely on inter-building distances and counts.

<!--preliminary modelling-->
As a first modelling step we tried a random forest (RF) classifier on a subset of the data.
The goal was to keep the final model as simple, scalable and interpretable as possible and evaluate the project workflow as a whole, rather than the specific model's performance.
We trained and tested the same simple RF model on the same data, split in different ways - one was based on stratified  spatial k-fold train/test splits (our proposed setup) and another based on random train/test splits.
...
The difference in accuracy highlights the extent of spatial leakage of information.
Furthermore, the relatively high accuracy score of the simple model hints towards the viability of predicting urban fabric types.


In total, the results point towards two things.
First, they further show need for a non-linear classification model, cable of accounting for the discrepancies between satellite-derived building footprint and cadastal data.
And second, the utility of our designed framework to account for spatial leakage and evaluate model performance in more realistic scenarios.
...
The figure below illustrates the spatial signatures class distribution, highlighting the imbalance across spatial signature types:

![](../figures/algo_design/unbalanced.png){.nostretch fig-align="center" height="420"}

A key challenge in our dataset is the class imbalance, where some urban fabric types are much more common than others. This imbalance required us to carefully consider how to design the model and choose the right loss function, prompting us to explore approaches that can handle uneven class distributions more effectively.
...
We conducted three main experiments as part of the AI model design to analyze urban fabric classification and segmentation.
  ...
  - Approach A (*Embedding approach*): We start with a baseline experiment, where we generate image embeddings using the SatlasPretrain model [@bastani2023satlaspretrain]. These embeddings are then fed into an XGBoost classifier to predict urban fabric classes.
...
The tiles are processed by the SatlasPretrain model, a geospatial foundation model pretrained on more than 302 million labels from remote sensing and computer vision tasks. We chose this model because it was specifically trained on Sentinel-2 images, making it a good fit for our dataset.
...
**Key observations**
  ...
  - The baseline classification approaches showed varied results:
      - The basic embedding classification approach achieved a global accuracy of 76% (66% balanced), reflecting a strong initial performance.
      - When incorporating H3 Level 5 spatial indexing, performance improved significantly, with a global accuracy of 87% (42% balanced), suggesting that regional context plays a critical role in improving classification accuracy.
  ...
  - The fine-tuned geospatial foundation model performed better than the classification models, achieving an accuracy of 0.73 compared to 0.56 for the classification model (Clay model).
