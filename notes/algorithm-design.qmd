...
![Example iteration in a five-fold cross validation split. Only one group is highlighted in blue, with its ETCs plotted in green, which will be used as a hold-out data for accuracy evaluation and hyper parameter tuning.](../figures/algo_design/train_test_prague.png){.nostretch fig-align="center" height="400"}
...
Nevertheless, there are urban fabric types present in the model which are ubiquitous in a lot of non-European contexts, such as communist planned areas in Asia, historical city centres, single housing areas and others.

A potential problem is class imbalance when training the model and after was when calculating its accuracy.
If such an issue is present in the country split experiments, we aim to address it by under-sampling the target classes.
We can afford to under-sample, rather than oversample, since our data covers a large geographical area and each class has tens of thousands of instances at the very least.
...
![](../figures/algo_design/train_df.png){.nostretch fig-align="left" height="300"}
![](../figures/algo_design/test_df.png){.nostretch fig-align="right" height="300"}
...
\AddToHookNext{env/longtable/begin}{\footnotesize}
